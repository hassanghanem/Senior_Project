...
$(document).on('click', '#logout', function () {
    ...
    var op = 3;
        $.ajax({
            type: 'GET',
            url: "../login_register/ws/users_ws.php?op=3",
            dataType: 'json',
            data: { op: op},
 ...
 if ( val.length < 10 ) {   
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            window.location.href = "Patient_formFilling.php";
            else {
                // alert("fi data");
        ...
        error: function (xhr, status, errorThrown) {
            alert(status + errorThrown +"redirect to data pagee");
        }
      });
  ...
  if (val.length < 10 ) {
    ...
    function parseImage1(response){
        ...
        document.getElementById("usrName").innerText = response[0].p_first_name.charAt(0).toUpperCase()  + response[0].p_first_name.slice(1);
    }
